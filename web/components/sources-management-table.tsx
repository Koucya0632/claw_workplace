...
export function SourcesManagementTable({
  ...
  return (
    <div className="overflow-x-auto rounded-[1.5rem] border border-slate-200 bg-white/94">
      <table className="min-w-full border-collapse text-left text-sm">
        <thead className="bg-sand/70">
          <tr className="text-[11px] uppercase tracking-[0.16em] text-slate-500">
            ...
            <th className="border-b border-slate-200 px-4 py-3">狀態</th>
            <th className="border-b border-slate-200 px-4 py-3">同步狀態</th>
            <th className="border-b border-slate-200 px-4 py-3">資料量</th>
            <th className="border-b border-slate-200 px-4 py-3">最後同步</th>
              ...
              <tr key={source.id} className="align-top even:bg-slate-50">
                <td className="border-b border-slate-200 px-4 py-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{source.type}</p>
                ...
                <td className="border-b border-slate-200 px-4 py-4">
                  <div className="flex flex-col items-start gap-2">
                    ...
                    <button
                      ...
                      className="pixel-button rounded-[0.9rem] bg-slate-100 px-3 py-2 text-xs font-black"
                    >
                      查看
                    </button>
                    <button
                      ...
                      className="pixel-button rounded-[0.9rem] bg-teal px-3 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {busyAction === syncKey ? "同步中" : "同步"}
                    </button>
                    </div>
                    <div className="flex flex-wrap gap-3 text-xs font-black text-slate-600">
                    <button type="button" onClick={() => onEdit(source)} className="underline decoration-2 underline-offset-2">
                      編輯
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className="text-coral disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      刪除
                    </button>
                    </div>
