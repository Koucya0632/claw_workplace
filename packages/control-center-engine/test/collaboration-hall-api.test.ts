...
test("GET /api/hall stays readable when the hall stores are missing", async () => {
  const backups = await backupFiles([
    COLLABORATION_HALLS_PATH,
    COLLABORATION_HALL_MESSAGES_PATH,
    COLLABORATION_TASK_CARDS_PATH,
  ...
  try {
    ...
    try {
      ...
      const hallResponse = await fetch(`${baseUrl}/api/hall`);
      assert.equal(hallResponse.status, 200);
      const hallPayload = await hallResponse.json() as {
        ok: boolean;
        hall: { hallId: string };
        taskCards: unknown[];
        messages: unknown[];
      };
      assert.equal(hallPayload.ok, true);
      assert.equal(hallPayload.hall.hallId, "main");
      assert.deepEqual(hallPayload.taskCards, []);
      assert.deepEqual(hallPayload.messages, []);
    } finally {
      if (server.listening) {
        await new Promise<void>((resolve, reject) => server.close((error) => (error ? reject(error) : resolve())));
      }
