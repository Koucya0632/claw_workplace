...
import { readCollaborationHall } from "../src/runtime/collaboration-hall-orchestrator";
...
test("reading the collaboration hall does not recreate the hall store on missing files", async () => {
  const hallsBefore = await readOptionalFile(COLLABORATION_HALLS_PATH);
  const messagesBefore = await readOptionalFile(COLLABORATION_HALL_MESSAGES_PATH);
  const taskCardsBefore = await readOptionalFile(COLLABORATION_TASK_CARDS_PATH);
  ...
  try {
    await Promise.all([
      rm(COLLABORATION_HALLS_PATH, { force: true }),
      rm(COLLABORATION_HALL_MESSAGES_PATH, { force: true }),
      rm(COLLABORATION_TASK_CARDS_PATH, { force: true }),
    ]);

    const result = await readCollaborationHall();

    assert.equal(result.hall.hallId, "main");
    assert.deepEqual(result.taskCards, []);
    assert.deepEqual(result.messages, []);
    assert.equal(await readOptionalFile(COLLABORATION_HALLS_PATH), undefined);
  } finally {
    await restoreOptionalFile(COLLABORATION_HALLS_PATH, hallsBefore);
    await restoreOptionalFile(COLLABORATION_HALL_MESSAGES_PATH, messagesBefore);
    await restoreOptionalFile(COLLABORATION_TASK_CARDS_PATH, taskCardsBefore);
