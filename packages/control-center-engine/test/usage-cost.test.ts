...
test("usage-cost snapshot marks disconnected sources with explicit placeholders", async () => {
  ...
  assert.equal(usage.budget.limitSource, "missing");
  assert(usage.budget.connectHint.includes("/api/budget/template"));
  assert.equal(usage.budget.templateHref, "/api/budget/template");
  assert(usage.budget.recommendedConfigPath?.replace(/\\/g, "/").endsWith("/runtime/usage-budget.json"));
  assert(usage.budget.actionLabel?.includes("monthly budget limit"));
...
test("usage-cost snapshot computes context percent and burn-rate status when sources exist", async () => {
  ...
  assert.equal(usage.budget.limitSource, "agent_budgets");
...
test("usage-cost snapshot falls back to global runtime limit when agent budgets are absent", async () => {
  const { computeUsageCostSnapshot } = await import("../src/runtime/usage-cost");
  const todayIso = new Date().toISOString().slice(0, 10);
  const snapshot = buildSnapshotFixture({ costLimit: undefined });
  ...
  assert.equal(usage.budget.limitSource, "global_runtime_limit");
  assert.equal(usage.budget.limitCost30d, 100);
  assert.equal(usage.budget.configPath, "/tmp/runtime/usage-budget.json");
});
...
test("usage-cost snapshot surfaces connected subscription consumption and remaining contract", async () => {
  ...
  assert.equal(usage.subscription.signalMode, "provider");
...
test("usage-cost snapshot surfaces unavailable subscription with explicit connector todo", async () => {
  ...
  assert.equal(usage.subscription.signalMode, "missing");
  assert.equal(usage.subscription.templateHref, "/api/subscription/template");
  assert(usage.subscription.recommendedSourcePath.replace(/\\/g, "/").endsWith("/runtime/subscription-snapshot.json"));
  assert(usage.subscription.sourceCandidates.length > 0);
  assert.equal(usage.connectors.subscriptionUsage, "not_connected");
  assert(usage.connectors.todos.some((item) => item.id === "subscription_usage"));
  assert(
    usage.connectors.todos.some(
      (item) => item.id === "subscription_usage" && !item.detail.includes("/tmp/subscription.json"),
    ),
  );
  assert(
    usage.connectors.todos.some(
      (item) => item.id === "cost_budget_limit" && !item.detail.includes("/runtime/usage-budget.json"),
    ),
  );
});
...
test("usage-cost snapshot backfills subscription consumed from runtime usage when provider snapshot is missing", async () => {
  ...
  assert.equal(usage.subscription.signalMode, "runtime_backfill");
  assert(usage.subscription.detail.includes("Remaining and total limit need a provider subscription snapshot"));
  ...
  assert(
    usage.connectors.todos.some(
      (item) =>
        ...
        item.detail.includes("Add a provider snapshot"),
    ),
  );
});
...
test("usage-cost snapshot backfills subscription limit and remaining from budget when provider snapshot is missing", async () => {
  ...
  assert.equal(usage.subscription.signalMode, "budget_backfill");
  assert(usage.subscription.sourcePath?.includes("budgetSummary"));
  assert(usage.subscription.detail.includes("Limit and remaining come from the configured monthly budget fallback"));
  assert.equal(usage.connectors.subscriptionUsage, "not_connected");
  assert(usage.connectors.todos.some((item) => item.id === "subscription_usage"));
});
