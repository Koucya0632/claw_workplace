...
test("staff overview role labels come from agent workspace evidence instead of global config model strings", async () => {
  const { buildStaffOverviewCards } = await import("../src/ui/server");
  const root = await mkdtemp(join(tmpdir(), "staff-role-evidence-"));

  try {
    const makeWorkspace = async (agentId: string, identityBody: string) => {
      const dir = join(root, agentId);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "IDENTITY.md"), identityBody, "utf8");
      return dir;
    };

    const fullstackWorkspace = await makeWorkspace(
      "fullstack-engineer-agent",
      `# IDENTITY.md

## Role

I am the specialist agent for engineering execution.
I own implementation, testing, and maintainable delivery.
`,
    );
    const dailyNewsWorkspace = await makeWorkspace(
      "daily-news-brief-agent",
      `# IDENTITY.md

## Role

I am the specialist agent for daily news monitoring, deduplication, ranking, and brief generation.
`,
    );
    const inspectionWorkspace = await makeWorkspace(
      "system-inspection-agent",
      `# IDENTITY.md

## Role

I am the specialist agent for system inspection, operational review, and upgrade risk assessment.
`,
    );
...
I am the specialist agent for searching, reading, and organizing information.
I focus on evidence quality and clean handoff.
`,
    );
    const mainWorkspace = await makeWorkspace(
      "main",
      `# IDENTITY.md

## Role Summary

I am Rex's main controller agent.
I own task intake, workflow routing, result integration, and final response framing.
`,
    );

    const cards = await buildStaffOverviewCards({
      snapshot: buildSnapshotFixture(),
      client: createIdleToolClient(),
      members: [
        { agentId: "main", displayName: "Secretary Controller", model: "gpt", workspace: mainWorkspace, toolsProfile: "coding" },
        { agentId: "support-agent", displayName: "Search & Organizer Agent", model: "gpt", workspace: supportWorkspace, toolsProfile: "coding" },
        { agentId: "daily-news-brief-agent", displayName: "Daily News Brief Agent", model: "gpt", workspace: dailyNewsWorkspace, toolsProfile: "coding" },
        { agentId: "system-inspection-agent", displayName: "System Inspection & Risk Assessment Agent", model: "gpt", workspace: inspectionWorkspace, toolsProfile: "coding" },
        { agentId: "fullstack-engineer-agent", displayName: "Fullstack Engineer Agent", model: "gpt", workspace: fullstackWorkspace, toolsProfile: "coding" },
      ],
      officeCards: [],
      executionAgentSummaries: [],
      language: "zh",
    ...
    assert.equal(cards.find((item) => item.agentId === "main")?.roleLabel, "主控与协调");
    assert.equal(cards.find((item) => item.agentId === "main")?.roleKey, "manager");
    assert.equal(cards.find((item) => item.agentId === "daily-news-brief-agent")?.roleLabel, "每日新闻与简报");
    assert.equal(cards.find((item) => item.agentId === "daily-news-brief-agent")?.roleKey, "planner");
    assert.equal(cards.find((item) => item.agentId === "system-inspection-agent")?.roleLabel, "系统巡检与风险评估");
    assert.equal(cards.find((item) => item.agentId === "system-inspection-agent")?.roleKey, "reviewer");
    assert.equal(cards.find((item) => item.agentId === "fullstack-engineer-agent")?.roleLabel, "工程交付与实现");
    assert.equal(cards.find((item) => item.agentId === "fullstack-engineer-agent")?.roleKey, "coder");
    assert.equal(cards.find((item) => item.agentId === "support-agent")?.roleLabel, "搜索与信息整理");
    assert.equal(cards.find((item) => item.agentId === "support-agent")?.roleKey, "planner");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("staff overview prefers the latest session state over older running residuals", async () => {
  const { buildStaffOverviewCards } = await import("../src/ui/server");
  const snapshot = buildSnapshotFixture();
  snapshot.sessions = [
    {
      ...
      agentId: "main",
      state: "idle",
      lastMessageAt: "2026-03-09T17:00:00.000Z",
    },
    {
      sessionKey: "agent:main:older-running",
      agentId: "main",
      state: "running",
      lastMessageAt: "2026-03-09T16:00:00.000Z",
    },
  ...
  const client: ToolClient = {
    ...createIdleToolClient(),
    async sessionsHistory(request) {
      if (request.sessionKey === "agent:main:latest") {
        return {
          json: {
            history: [
              { role: "assistant", content: "已完成整理，等待下一步指令。" },
            ],
          },
          rawText: "",
        };
      }
      if (request.sessionKey === "agent:main:older-running") {
        return {
          json: {
            history: [
              { role: "assistant", content: "正在继续处理旧任务上下文。" },
            ],
          },
          rawText: "",
        };
  ...
  const cards = await buildStaffOverviewCards({
    ...
    client,
    members: [
      { agentId: "main", displayName: "Secretary Controller", model: "gpt", workspace: "main", toolsProfile: "coding" },
    ],
    officeCards: [
      {
        agentId: "main",
    ...
    executionAgentSummaries: [
      { agentId: "main", displayName: "main", activeSessions: 1, activeTasks: 0, enabledCronJobs: 0, cronJobNames: [], recentTokens30d: 0 },
    ],
    language: "zh",
  });
  ...
  assert(main);
  assert.equal(main.statusLabel, "待命");
  assert.equal(main.statusSource, "latest_session");
  assert.equal(main.currentWork, "当前无实时任务");
  assert.equal(main.recentOutput, "已完成整理，等待下一步指令。");
});

function createIdleToolClient(): ToolClient {
  return {
    async sessionsList() { return { sessions: [] }; },
    async sessionStatus() { return { rawText: "" }; },
    async sessionsHistory() { return { rawText: "" }; },
