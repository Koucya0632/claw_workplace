...
import { APPROVAL_ACTIONS_ENABLED, GATEWAY_TOKEN, GATEWAY_URL } from "../config";
...
async function runText(
  ...
  const commandArgs = [...args, ...buildGatewayConnectionArgs()];
  const { stdout } = await execFileAsync("openclaw", commandArgs, {
...
async function runStreamingText(
  ...
  return await new Promise((resolve, reject) => {
    const child = spawn("openclaw", commandArgs, {
      shell: process.platform === "win32",
...
async function readCronJobsFromStore(): Promise<CronListResponse> {
  try {
    const openclawHome = resolveOpenClawHomePath();
    const raw = await readFile(join(openclawHome, "cron", "jobs.json"), "utf8");
...
function buildGatewayConnectionArgs(): string[] {
  const args: string[] = [];
  const normalizedUrl = normalizeGatewayCliUrl(GATEWAY_URL);
  if (normalizedUrl) args.push("--url", normalizedUrl);
  if (GATEWAY_TOKEN) args.push("--token", GATEWAY_TOKEN);
  return args;
}

function normalizeGatewayCliUrl(gatewayUrl: string): string {
