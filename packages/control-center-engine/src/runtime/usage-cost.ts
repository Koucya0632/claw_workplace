...
const DEFAULT_USAGE_BUDGET_PATH = join(RUNTIME_DIR, "usage-budget.json");
...
let usageBudgetConfigCache: TimedSourceCache<UsageBudgetConfig> | undefined;
let usageBudgetConfigInFlight: Promise<UsageBudgetConfig> | undefined;
...
export async function buildUsageCostSnapshot(
  ...
  if (mode === "summary") {
    const [digests, subscriptionUsage, usageBudgetConfig] = await Promise.all([
      loadCachedUsageDigests(),
      loadCachedSubscriptionUsage({ includeCodexTelemetry: false }),
      loadCachedUsageBudgetConfig(),
    ]);
    return computeUsageCostSnapshot(snapshot, digests, [], undefined, subscriptionUsage, new Map(), usageBudgetConfig);
  }

  const [digests, modelCatalog, runtimeUsage, subscriptionUsage, cronJobNameMap, usageBudgetConfig] = await Promise.all([
    loadCachedUsageDigests(),
  ...
  return computeUsageCostSnapshot(
    snapshot,
    digests,
    modelCatalog,
    runtimeUsage,
...
export function computeUsageCostSnapshot(
  ...
  usageBudgetConfig?: UsageBudgetConfig,
): UsageCostSnapshot {
  ...
  const budget = buildUsageBudgetStatus(snapshot, period30, usageBudgetConfig);
  ...
  const connectorStatus = buildConnectorStatus({
    ...
    budgetConnectHint: budget.connectHint,
    ...
    subscriptionConnectHintShort: subscription.connectHintShort,
...
async function loadCachedUsageBudgetConfig(): Promise<UsageBudgetConfig> {
  const now = Date.now();
  if (usageBudgetConfigCache && usageBudgetConfigCache.expiresAt > now) {
    return usageBudgetConfigCache.value;
  }
  if (!usageBudgetConfigInFlight) {
    usageBudgetConfigInFlight = loadUsageBudgetConfig()
      .then((value) => {
        usageBudgetConfigCache = {
          value,
          expiresAt: Date.now() + USAGE_SOURCE_CACHE_TTL_MS,
        };
        return value;
      ...
      .finally(() => {
        usageBudgetConfigInFlight = undefined;
      });
  }
  return usageBudgetConfigInFlight;
}

async function loadUsageBudgetConfig(): Promise<UsageBudgetConfig> {
  const configPath = DEFAULT_USAGE_BUDGET_PATH;

  try {
    const raw = JSON.parse(await readFile(configPath, "utf8")) as Record<string, unknown>;
    const monthlyCostLimitUsd = asFiniteNumber(raw.monthlyCostLimitUsd);
    if (monthlyCostLimitUsd !== undefined && monthlyCostLimitUsd > 0) {
      return {
        monthlyCostLimitUsd,
        configPath,
      };
    ...
    return {
      configPath,
      detail: `Budget config found at ${configPath}, but monthlyCostLimitUsd is missing or invalid.`,
    };
  } catch (error) {
    ...
    return {
      configPath,
      detail: `Budget config at ${configPath} is unreadable${readErrorCode(error) ? ` (${readErrorCode(error)})` : ""}.`,
    };
  }
}
...
function buildUsageBudgetStatus(
  ...
  const limitCost30d =
    agentCostLimits.length > 0
      ? agentCostLimits.reduce((sum, value) => sum + value, 0)
      : usageBudgetConfig?.monthlyCostLimitUsd;

  if (!limitCost30d || !Number.isFinite(limitCost30d) || limitCost30d <= 0) {
    const configPath = usageBudgetConfig?.configPath ?? DEFAULT_USAGE_BUDGET_PATH;
    return {
      status: "not_connected",
      usedCost30d,
      limitSource: "missing",
      message: "Monthly budget limit is not configured yet.",
      detail:
        usageBudgetConfig?.detail?.trim() ||
        "No agent cost thresholds or global monthly cost limit are configured yet.",
      connectHint: budgetConnectHint(configPath),
      configPath,
      recommendedConfigPath: configPath,
  ...
  const limitSource =
    agentCostLimits.length > 0 ? "agent_budgets" : "global_runtime_limit";
  ...
  const detail =
    limitSource === "agent_budgets"
      ? "Monthly burn is evaluated against aggregated agent cost thresholds."
      : `Monthly burn is evaluated against the global runtime limit in ${usageBudgetConfig?.configPath ?? DEFAULT_USAGE_BUDGET_PATH}.`;
  const connectHint =
    limitSource === "agent_budgets"
      ? "Add runtime/usage-budget.json only if you need a global fallback limit."
      : budgetConnectHint(usageBudgetConfig?.configPath ?? DEFAULT_USAGE_BUDGET_PATH);
  ...
  if (ratio >= 1) {
    return {
      ...
      detail,
      connectHint,
      configPath: limitSource === "global_runtime_limit" ? usageBudgetConfig?.configPath ?? DEFAULT_USAGE_BUDGET_PATH : undefined,
      recommendedConfigPath:
        limitSource === "global_runtime_limit" ? usageBudgetConfig?.configPath ?? DEFAULT_USAGE_BUDGET_PATH : undefined,
      templateHref: "/api/budget/template",
      actionLabel:
        limitSource === "agent_budgets"
          ? "Monthly budget is enforced by aggregated agent cost thresholds."
          : "Global runtime budget limit is active.",
    };
  }
  if (ratio >= BUDGET_WARN_RATIO) {
    return {
      ...
      actionLabel:
        limitSource === "agent_budgets"
          ? "Monthly budget is tracked against aggregated agent cost thresholds."
          : "Global runtime budget limit is active.",
    };
  }
  return {
...
function buildConnectorStatus(input: {
  ...
  if (!input.hasSubscriptionSignal) {
    todos.push({
      ...
      detail: input.subscriptionConnectHintShort || input.subscriptionConnectHint,
    });
  } else if (!input.hasSubscriptionConnected) {
    todos.push({
      id: "subscription_usage",
      title: subscriptionTodoTitle(input.subscriptionReasonCode),
      detail: subscriptionTodoDetail(
        input.subscriptionReasonCode,
        input.subscriptionDetail,
        input.subscriptionConnectHintShort || input.subscriptionConnectHint,
      ),
    });
  }
...
function subscriptionTodoTitle(reasonCode: SubscriptionReasonCode | undefined): string {
  ...
  if (reasonCode === "runtime_backfill_only" || reasonCode === "runtime_backfill_with_provider_partial") {
    return "Connect provider subscription snapshot";
  }
  if (reasonCode === "runtime_backfill_with_budget_limit") return "Replace estimated subscription signal";
...
function subscriptionTodoDetail(
  ...
  if (reasonCode === "provider_snapshot_unreadable") {
    return "The subscription snapshot cannot be parsed. Regenerate it from /api/subscription/template or fix the file in place.";
  }
  if (reasonCode === "runtime_backfill_only") {
    return "Used is estimated from runtime usage only. Add a provider snapshot to reveal remaining and total limit.";
  }
  if (reasonCode === "runtime_backfill_with_budget_limit") {
    return "Used is estimated from runtime usage and limit comes from the configured budget fallback. Add a provider snapshot for real provider remaining/limit.";
  }
  const normalizedDetail = detail.trim();
  if (normalizedDetail) return normalizedDetail;
  return connectHintShort || "Subscription usage fields are incomplete.";
}
...
async function loadSubscriptionUsage(options: { includeCodexTelemetry?: boolean } = {}): Promise<UsageSubscriptionStatus> {
  ...
  if (unreadablePaths.length > 0) {
    ...
    return decorateSubscriptionUsage({
      status: "partial",
      planLabel: "Subscription data unreadable",
      unit: "USD",
      detail: "The subscription snapshot exists but cannot be parsed yet.",
...
function finalizeSubscriptionUsage(
  ...
  if (!subscriptionUsage || subscriptionUsage.status === "not_connected") {
    if (runtimeConsumed === undefined) {
      return subscriptionUsage ? decorateSubscriptionUsage(subscriptionUsage) : defaultSubscriptionUsage();
    }
    if (budgetLimit !== undefined) {
      ...
      return decorateSubscriptionUsage({
        ...
        detail: "Used is estimated from runtime usage. Limit and remaining come from the configured monthly budget fallback.",
    ...
    return decorateSubscriptionUsage({
      ...
      detail: "Used is estimated from runtime usage. Remaining and total limit need a provider subscription snapshot.",
      connectHint: providerConnectHint,
  ...
  if (providerConnected) return decorateSubscriptionUsage(subscriptionUsage);
  ...
  return decorateSubscriptionUsage({
    ...
    detail: detail || "Subscription data is partially connected.",
    connectHint: providerConnectHint,
    reasonCode,
  });
}
...
async function loadCodexRateLimitUsage(connectHint: string): Promise<UsageSubscriptionStatus | undefined> {
  ...
  return {
    ...buildSubscriptionSetupFields("provider_connected"),
    status: "connected",
    ...
    connectHint,
    connectHintShort: "Codex quota telemetry is connected.",
    sourceCandidates: SUBSCRIPTION_SNAPSHOT_PATHS,
    templateHref: "/api/subscription/template",
    templateSavePath: DEFAULT_SUBSCRIPTION_SNAPSHOT_PATH,
    signalMode: "provider",
...
function budgetConnectHint(configPath: string): string {
  return `Use /api/budget/template and save it as ${configPath}, or add cost thresholds to agent budgets.`;
}
...
function buildSubscriptionSetupFields(
  reasonCode: SubscriptionReasonCode | undefined,
): Pick<
  UsageSubscriptionStatus,
  "connectHintShort" | "sourceCandidates" | "templateHref" | "templateSavePath" | "recommendedSourcePath" | "signalMode"
> {
  return {
    connectHintShort: resolveSubscriptionConnectHintShort(reasonCode),
    sourceCandidates: SUBSCRIPTION_SNAPSHOT_PATHS,
    templateHref: "/api/subscription/template",
    templateSavePath: DEFAULT_SUBSCRIPTION_SNAPSHOT_PATH,
    recommendedSourcePath: DEFAULT_SUBSCRIPTION_SNAPSHOT_PATH,
    signalMode: resolveSubscriptionSignalMode(reasonCode),
  };
}

function decorateSubscriptionUsage(
  value: Omit<
    UsageSubscriptionStatus,
    "connectHintShort" | "sourceCandidates" | "templateHref" | "templateSavePath" | "recommendedSourcePath" | "signalMode"
  >,
  ...
  return {
    ...value,
    ...buildSubscriptionSetupFields(value.reasonCode),
  };
}

function resolveSubscriptionSignalMode(
  reasonCode: SubscriptionReasonCode | undefined,
): UsageSubscriptionStatus["signalMode"] {
  if (reasonCode === "provider_connected" || reasonCode === "provider_snapshot_partial") return "provider";
  if (reasonCode === "runtime_backfill_with_budget_limit") return "budget_backfill";
  if (reasonCode === "runtime_backfill_only" || reasonCode === "runtime_backfill_with_provider_partial") {
...
function resolveSubscriptionConnectHintShort(reasonCode: SubscriptionReasonCode | undefined): string {
  if (reasonCode === "provider_connected") return "Provider subscription telemetry is connected.";
  if (reasonCode === "provider_snapshot_unreadable") {
    return "Fix the unreadable subscription snapshot or regenerate it from /api/subscription/template.";
  }
  if (reasonCode === "provider_snapshot_partial") {
    return "Complete the provider snapshot fields or regenerate it from /api/subscription/template.";
  }
  if (reasonCode === "runtime_backfill_only") {
    return "Used is estimated from runtime usage. Add a provider snapshot to show remaining and total limit.";
  }
  if (reasonCode === "runtime_backfill_with_budget_limit") {
    return "Used is estimated from runtime usage and limit comes from the configured budget fallback.";
  }
  if (reasonCode === "runtime_backfill_with_provider_partial") {
    return "Used is estimated from runtime usage until the provider snapshot is complete.";
  }
  return "Create a provider snapshot via /api/subscription/template or connect Codex telemetry.";
}
  ...
  if (!hasSignal) {
    if (isSyntheticSnapshot) {
      return decorateSubscriptionUsage({
        ...
        cycleEnd,
        sourcePath,
        detail: "A local estimated snapshot is present, but strict mode waits for provider billing fields.",
        connectHint,
        reasonCode: "runtime_backfill_with_budget_limit",
      });
    }
    return decorateSubscriptionUsage({
      ...
      cycleEnd,
      sourcePath,
      detail: "A provider snapshot is present, but consumed, remaining, or limit fields are still incomplete.",
      connectHint,
...
function defaultSubscriptionUsage(missingPaths: string[] = SUBSCRIPTION_SNAPSHOT_PATHS): UsageSubscriptionStatus {
  return decorateSubscriptionUsage({
    ...
    unit: "USD",
    detail: "No provider subscription snapshot is connected yet.",
    sourcePath: missingPaths.join("; "),
    connectHint: subscriptionConnectHint(),
