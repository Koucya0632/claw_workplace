...
export interface EnsureDefaultCollaborationHallOptions {
  persist?: boolean;
}
...
export async function ensureDefaultCollaborationHall(
  participants: HallParticipant[],
  options: EnsureDefaultCollaborationHallOptions = {},
  ...
  if (existing) {
    ...
    existing.participants = nextHall.participants;
    existing.updatedAt = nextHall.updatedAt;
    store.updatedAt = now;
