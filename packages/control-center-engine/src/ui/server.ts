...
type StaffRoleGroupKey = "manager" | "planner" | "coder" | "reviewer" | "generalist" | "unassigned";

interface StaffSummaryRoleGroup {
  roleKey: StaffRoleGroupKey;
  roleLabel: string;
  count: number;
  members: Array<{
...
export function startUiServer(port: number, toolClient: ToolClient, options: StartUiServerOptions = {}): Server {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (method === "GET" && path === "/api/staff-summary") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const snapshot = await readReadModelSnapshotWithLiveSessions(toolClient);
        const allTasks = listTasks(snapshot.tasks, projectTitleMap(snapshot)).filter((task) => !isControlCenterMappingTask(task));
        const [usageCost, officeRoster, officePresence, openclawCronJobs] = await Promise.all([
          loadCachedUsageCost(snapshot, "full"),
          loadBestEffortAgentRoster(),
          loadCachedOfficeSessionPresence(),
          loadOpenclawCronCatalog("zh"),
        ]);
        const teamSnapshot = await loadTeamSnapshot(officeRoster, snapshot);
        ...
        const sessions = await listSessionConversations({
          ...
          filters: {},
          page: 1,
          pageSize: Math.max(snapshot.sessions.length, 24),
          historyLimit: 20,
        });

        return writeJson(res, 200, buildStaffSummaryResponse({
      ...
      if (method === "GET" && path === "/api/files") {
        ...
        const facetOptions = scope === "workspace" ? await listWorkspaceFacetOptions() : await listMemoryFacetOptions();
        return writeJson(res, 200, {
          ok: true,
          scope,
          count: files.length,
      ...
      if (method === "GET" && path === "/api/budget/template") {
        assertAllowedQueryParams(url.searchParams, [], true);
        return writeJson(res, 200, {
          ok: true,
          template: {
          ...
          hint: `Save as ${join(process.cwd(), "runtime", "usage-budget.json")}`,
        });
      }
      ...
      if (method === "GET" && path === "/cron") {
        ...
        return writeJson(res, 200, {
          ok: true,
          overview,
          rows: buildCronOverviewRows(overview, catalog, language),
        });
      }
...
function cronPayloadOwner(
  payload: Record<string, unknown> | undefined,
  language: UiLanguage,
  job?: Record<string, unknown> | undefined,
): string {
  if (!payload && !job) return pickUiText(language, "Scheduler", "调度器");
  const ownerAgentId = cronPayloadOwnerAgentId(payload, job);
  if (ownerAgentId) return humanizeOperatorLabel(ownerAgentId);
  const sessionTarget = payload && typeof payload.sessionTarget === "string" ? payload.sessionTarget : undefined;
  const sessionOwner = parseSessionTargetOwner(sessionTarget);
  if (sessionOwner) return humanizeOperatorLabel(sessionOwner);
  return pickUiText(language, "Scheduler", "调度器");
}

function cronPayloadOwnerAgentId(
  payload: Record<string, unknown> | undefined,
  job?: Record<string, unknown> | undefined,
): string | undefined {
  if (job && typeof job.agentId === "string" && job.agentId.trim()) {
    ...
    try {
      ...
      for (const item of jobsRaw) {
        ...
        jobs.push({
          ...
          owner: cronPayloadOwner(payload, language, obj),
          ownerAgentId: cronPayloadOwnerAgentId(payload, obj),
          purpose: cronPayloadPurpose(payload, language),
          scheduleLabel: cronScheduleLabel(schedule, language),
          sourcePath: candidate,
          lastRunAt: normalizeCronStateDate(state?.lastRunAtMs),
          lastRunStatus: typeof state?.lastRunStatus === "string" ? state.lastRunStatus.trim() : undefined,
          lastError: typeof state?.lastError === "string" && state.lastError.trim() ? state.lastError.trim() : undefined,
        });
      }
      jobs.sort((a, b) => {
...
function buildCronOverviewRows(
  overview: Awaited<ReturnType<typeof buildCronOverview>>,
  catalog: OpenclawCronJobSummary[],
  language: UiLanguage,
): CronOverviewApiRow[] {
  const catalogById = new Map(catalog.map((job) => [job.jobId, job]));
  const overviewRows = overview.jobs.map((job) => {
    const catalogJob = catalogById.get(job.jobId);
    const heartbeat = isHeartbeatJob(job.jobId);
    return {
      jobId: job.jobId,
      name: catalogJob?.name ?? job.name ?? job.jobId,
      channel: heartbeat ? "heartbeat" : "cron",
      enabled: job.enabled,
      owner:
        catalogJob?.owner ??
        formatExecutorAgentLabel(heartbeat ? "task-heartbeat-worker" : "system-cron", language),
      ownerAgentId: catalogJob?.ownerAgentId,
      purpose:
        catalogJob?.purpose ??
        (heartbeat
          ? pickUiText(language, "Check task heartbeat and pickup cadence.", "检查任务心跳与拾取节奏。")
          : cronRuntimePurpose(job.jobId, language)),
      schedule: catalogJob?.scheduleLabel ?? pickUiText(language, "system interval", "系统间隔"),
      nextRunAt: job.nextRunAt,
      status: job.health,
      lastRunAt: catalogJob?.lastRunAt,
      lastRunStatus: normalizeCronRunStatus(catalogJob?.lastRunStatus),
      lastError: catalogJob?.lastError,
    } satisfies CronOverviewApiRow;
  });

  const seenJobIds = new Set(overviewRows.map((row) => row.jobId));
  const catalogOnlyRows = catalog
      ...
      return {
        ...
        purpose:
          ...
          (heartbeat
            ? pickUiText(language, "Check task heartbeat and pickup cadence.", "检查任务心跳与拾取节奏。")
            : cronRuntimePurpose(job.jobId, language)),
        schedule: job.scheduleLabel ?? pickUiText(language, "system interval", "系统间隔"),
        nextRunAt: undefined,
        status: job.enabled ? "scheduled" : "disabled",
        lastRunAt: job.lastRunAt,
        lastRunStatus: normalizeCronRunStatus(job.lastRunStatus),
        lastError: job.lastError,
      } satisfies CronOverviewApiRow;
    });

  return [...overviewRows, ...catalogOnlyRows];
}

function isHeartbeatJob(jobId: string): boolean {
  return jobId.trim().toLowerCase().includes("heartbeat");
}

function normalizeCronStateDate(value: unknown): string | undefined {
  return typeof value === "number" && Number.isFinite(value) ? new Date(value).toISOString() : undefined;
}

function normalizeCronRunStatus(status: string | undefined): string | undefined {
  const normalized = status?.trim().toLowerCase();
  if (!normalized) return undefined;
  if (["ok", "success", "succeeded", "completed", "finished"].includes(normalized)) return "success";
  if (["error", "failed", "failure", "timeout"].includes(normalized)) return "error";
  return normalized;
}
...
function extractMarkdownSection(input: string, headings: string[]): string | undefined {
  ...
  for (const rawLine of lines) {
    ...
    if (isHeading) {
      if (collecting) break;
      if (headings.some((heading) => normalized === heading.toLowerCase())) {
        collecting = true;
      }
      continue;
    }
...
async function resolveStaffRoleLabel(member: TeamMemberSnapshot, language: UiLanguage = "zh"): Promise<string> {
  ...
  const roleSections = roleEvidence
    .map((entry) => extractMarkdownSection(entry, ["Role", "职责", "角色", "Mission", "任务", "目标"]))
  ...
  if (
    key === "main" ||
    normalized.includes("main controller agent") ||
    normalized.includes("chief coordinator") ||
    normalized.includes("controller role") ||
    normalized.includes("secretary controller") ||
    normalized.includes("this workspace belongs to the main controller agent")
  ) {
    return pickUiText(language, "Main control and coordination", "主控与协调");
  }

  if (
    key === "fullstack-engineer-agent" ||
    normalized.includes("engineering execution") ||
    normalized.includes("specialist agent for engineering execution") ||
    normalized.includes("implementation starts") ||
    normalized.includes("testing and verification") ||
    normalized.includes("maintainable technical solutions")
  ) {
    return pickUiText(language, "Engineering execution and delivery", "工程交付与实现");
  }

  if (
    key === "daily-news-brief-agent" ||
    normalized.includes("daily news monitoring") ||
    normalized.includes("brief generation") ||
    normalized.includes("news monitoring") ||
    normalized.includes("deduplication") ||
    normalized.includes("daily brief")
  ) {
    return pickUiText(language, "Daily news and briefings", "每日新闻与简报");
  }

  if (
    key === "support-agent" ||
    normalized.includes("searching, reading, and organizing information") ||
    normalized.includes("search and organize specialist") ||
    normalized.includes("knowledge acquisition") ||
    normalized.includes("organize evidence") ||
    normalized.includes("evidence quality")
  ) {
    return pickUiText(language, "Evidence search and organization", "搜索与信息整理");
  }

  if (
    key === "system-inspection-agent" ||
    normalized.includes("system inspection") ||
    normalized.includes("risk assessment") ||
    normalized.includes("upgrade risk") ||
    normalized.includes("operational review")
  ) {
    return pickUiText(language, "System inspection and risk assessment", "系统巡检与风险评估");
  }
...
async function loadStaffRecentActivity(
  ...
  const entries = await Promise.all(
    targetKeys.map(async (key): Promise<[string, StaffRecentActivity | undefined]> => {
      ...
      let fallbackRecent: StaffRecentActivity | undefined;
      let primarySessionKey: string | undefined;
      let primaryEffectiveState: OfficeSpaceCard["status"] | undefined;
      let primaryRecent: StaffRecentActivity | undefined;
      let primaryResidualRunning = false;
      ...
      for (const [index, session] of sessions.entries()) {
        ...
        const recent = detail ? pickRecentStaffActivity(history, language) : undefined;

        if (!fallbackRecent && recent) {
          fallbackRecent = {
            ...recent,
            sessionKey: session.sessionKey,
            statusSource: "latest_session",
          };
        }

        const sessionState = detailSession?.state ?? session.state;
        const residualRunning = sessionState === "running" && historyImpliesStaffStopped(history);
        const effectiveState = residualRunning ? "idle" : sessionState;

        if (index === 0) {
          primarySessionKey = session.sessionKey;
          primaryEffectiveState = effectiveState;
          primaryRecent = recent;
          primaryResidualRunning = residualRunning;
        }
      }

      if (!primarySessionKey || !primaryEffectiveState) {
        return [key, fallbackRecent];
      }

      if (primaryEffectiveState === "running") {
        return [
          key,
          {
            ...(primaryRecent ?? fallbackRecent ?? {
              recentOutput: pickUiText(language, "Currently active with no recent visible output.", "当前正在处理任务，但最近没有可见产出。"),
            }),
      ...
      return [
        ...
        primaryRecent
          ...
          : fallbackRecent
            ...
            : {
                recentOutput: primaryResidualRunning
                  ? pickUiText(language, "Recently stopped and returned to standby.", "最近已停止当前任务并回到待命。")
                  : pickUiText(language, "No recent output yet.", "最近暂无产出。"),
                sessionKey: primarySessionKey,
                statusOverride: "idle",
                statusSource: "latest_session",
              },
...
function resolveStaffRoleKey(member: TeamMemberSnapshot, roleLabel: string): StaffRoleGroupKey {
  const key = normalizeLookupKey(member.agentId);
  const normalized = normalizeEvidenceText(`${member.customNote ?? ""}\n${roleLabel}`);

  if (
    key === "main" ||
    normalized.includes("coordination") ||
    normalized.includes("control and coordination") ||
    normalized.includes("主控") ||
    normalized.includes("协调")
  ) {
    return "manager";
  }
  ...
  if (
    ...
    normalized.includes("daily news") ||
    normalized.includes("search and organization") ||
    normalized.includes("evidence search") ||
    normalized.includes("信息整理") ||
    normalized.includes("搜索") ||
    normalized.includes("简报") ||
    normalized.includes("情报")
  ) {
    return "planner";
  }
  ...
  if (
    ...
    key === "fullstack-engineer-agent" ||
    normalized.includes("engineering execution") ||
    normalized.includes("engineering delivery") ||
  ...
  if (
    normalized.includes("role not defined") ||
    normalized.includes("未写明职责")
  ) {
    return "unassigned";
  }
...
export async function buildStaffOverviewCards(input: {
  ...
  return await Promise.all(memberList.map(async (member) => {
    ...
    return {
      ...
      identity,
      roleKey: resolveStaffRoleKey(member, roleLabel),
      roleLabel,
      statusLabel: staffStatusLabel(effectiveOfficeStatus, input.language),
      statusSource: recentActivity?.statusSource ?? "office_fallback",
      currentWorkLabel: currentWork.label,
      currentWork: currentWork.value,
      recentOutput,
      recentOutputAt: recentActivity?.recentOutputAt,
      scheduledLabel,
    };
  }));
}
...
function staffRoleGroupLabel(roleKey: StaffRoleGroupKey): string {
  if (roleKey === "manager") return "Manager";
  if (roleKey === "planner") return "Planner";
  if (roleKey === "coder") return "Coder";
  if (roleKey === "reviewer") return "Reviewer";
  if (roleKey === "generalist") return "Generalist";
  return "Unassigned";
}

function staffRoleGroupRank(roleKey: StaffRoleGroupKey): number {
  if (roleKey === "manager") return 0;
  if (roleKey === "planner") return 1;
  if (roleKey === "coder") return 2;
  if (roleKey === "reviewer") return 3;
  if (roleKey === "generalist") return 4;
  return 5;
}

export function buildStaffSummaryResponse(input: {
  generatedAt: string;
  cards: StaffOverviewCard[];
  sessions: SessionConversationListItem[];
}): StaffSummaryResponse {
  const groupsByKey = new Map<StaffRoleGroupKey, StaffSummaryRoleGroup>();

  for (const card of input.cards) {
    const group = groupsByKey.get(card.roleKey) ?? {
      roleKey: card.roleKey,
      roleLabel: staffRoleGroupLabel(card.roleKey),
      count: 0,
      members: [],
    };
    group.members.push({
      ...
      statusSource: card.statusSource,
      currentWorkLabel: card.currentWorkLabel,
      currentWork: card.currentWork,
      recentOutput: card.recentOutput,
      recentOutputAt: card.recentOutputAt,
      scheduledLabel: card.scheduledLabel,
    });
    group.count = group.members.length;
    groupsByKey.set(card.roleKey, group);
  }

  const groups = [...groupsByKey.values()]
    .sort((a, b) => {
      const rank = staffRoleGroupRank(a.roleKey) - staffRoleGroupRank(b.roleKey);
      if (rank !== 0) return rank;
      return a.roleLabel.localeCompare(b.roleLabel, "en");
    })
    .map((group) => ({
      ...group,
      members: [...group.members].sort((a, b) => a.displayName.localeCompare(b.displayName, "zh-Hans-CN")),
    }));
  ...
  return {
    ...
    sessionsDetail: {
      ...
      sessions: input.sessions.map((session) => ({
        ...
        updatedAt: session.latestHistoryAt ?? session.lastMessageAt,
        latestSnippet: session.latestSnippet,
      })),
    },
  };
}
